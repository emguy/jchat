...
public class ChatServer extends AbstractServer{
	...
	public void handleMessageFromClient(Object msg, ConnectionToClient client){
		if(client.getInfo("ID") != null || msg.toString().substring(0,7).equals("#login ") || msg.toString().substring(0,5).equals("#reg ") || msg.toString().substring(0,9).equals("#regInfo ")){
	...
	public void handleMessageFromServerUI(String message){
		if(message.charAt(0)=='#'){
			...
			if(serverOn)
				...
				else serverUI.display("<System> Server is down. Message cannot be sent.");
			}
	}
		...
		try{
			if(command[0].equals("#quit")){
				...
				if(serverOn){
					...
					else serverUI.display("<System> Listening has already stopped.");
				}
				else serverUI.display("<System> Server is down now.");
			}
				...
				else serverUI.display("<System> Server has already been closed.");
			}
				...
				if(serverOn){
					if(this.isListening()) serverUI.display("<System> Server already started.");
					else{
						listen();
			...
			else serverUI.display("Command not found");
		}
		catch(Exception e){
