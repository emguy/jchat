...
public class ChatClient extends AbstractClient {
  ...
  public void handleMessageFromClientUI(String message) {
    if (message.charAt(0) == '#') {
      ...
      if (isLogin) {
        ...
        clientUI.display("<System> Not Loged in");
    }
  }
    ...
    try {
      if (command[0].equals("#quit")) {
        ...
        if (isLogin) {
          ...
          clientUI.display("<System> You have already logged off.");
      } else if (command[0].equals("#sethost")) {
        if (isLogin)
          clientUI.display("<System> Please log off first");
        else {
        ...
        if (isLogin)
          clientUI.display("<System> You have already logged in.");
        else {
          try {
            this.openConnection();
        ...
        if (isLogin)
          ...
          clientUI.display("<System> You are currently not connected.");
        ...
        clientUI.display("<System> Command not found.");
    } catch (IOException e) {
  ...
  public void connectionException(Exception exception) {
    try {
      this.closeConnection();
      isLogin = false;
      clientUI.display("<System> The connection to the Server (" + this.getHost() + ":" + this.getPort()
